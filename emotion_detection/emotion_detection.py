...
class predict_emotions():
    def __init__(self):
        f = open(CONF_PATH)
        self.cfg = json.load(f)['emotion_detection']
        self.model = load_model(self.cfg['emotion_model'])
    ...
    def get_emotion(self,img,boxes_face):
        ...
        if len(boxes_face)!=0:
            for box in boxes_face:
                ...
                face_image = self.preprocess_img(face_image ,self.cfg['rgb'], self.cfg['width'], self.cfg['height'])

                prediction = self.model.predict(face_image)
                emotion = self.cfg['labels'][prediction.argmax()]
